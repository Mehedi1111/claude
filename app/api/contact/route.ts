...
export async function POST(request: Request) {
  ...
  // Sanitize for safe use in email HTML and SMTP subject
  ...
  const html = `
    <div style="font-family: 'Helvetica Neue', sans-serif; max-width: 600px; margin: 0 auto; color: #0a0a0a;">
      ...
      <div style="padding: 32px 0; border-bottom: 1px solid #e5e5e5;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          ${safeAiTool ? `<tr>
            ...
            <td style="padding: 8px 0; font-size: 14px;">${safeAiTool}</td>
          </tr>` : ""}
          ${safeFileName ? `<tr>
            <td style="padding: 8px 0; font-size: 12px; color: #737373; text-transform: uppercase; letter-spacing: 0.1em;">File</td>
            <td style="padding: 8px 0; font-size: 14px;">${safeFileName}</td>
          </tr>` : ""}
        </table>
      </div>
      ...
      <div style="padding: 32px 0; border-bottom: 1px solid #e5e5e5;">
        ...
        <p style="font-size: 15px; line-height: 1.6; margin: 0; color: #404040;">${safeMessage.replace(/\n/g, "<br>")}</p>
      </div>` : ""}
