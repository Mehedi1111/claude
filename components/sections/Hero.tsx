...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // Only load GSAP on desktop — it's a 45 KB library used solely for
    // the parallax scroll effect, which mobile doesn't need.
    if (mobile || !sectionRef.current || !contentRef.current) return;
  ...
  return (
    ...
    >
      ...
      {/* Mobile: static Clutch badge — no JS animation */}
      {isMobile && (
        <div className="absolute top-24 right-6 text-right hidden sm:block">
          <p className="text-[10px] font-sans text-white/20 uppercase tracking-[0.2em] mb-1.5">Rated on Clutch</p>
          <div className="flex items-center gap-0.5 justify-end">
            {[1, 2, 3, 4, 5].map((s) => (
              <span key={s} className="text-white/35 text-[11px]">★</span>
      ...
      >
        ...
        <div className="mb-10 sm:mb-12">
          {headline.map((line, i) => (
            ...
            >
              {isMobile ? (
                <div
                  className={`font-display font-bold tracking-[-0.04em] text-white text-[clamp(44px,7.5vw,115px)] ${
                    i === 2 ? "inline-block mr-[0.3em]" : ""
                  }`}
                >
                  {line.text}
                  {i === 2 && <span className="text-white"> Authority.</span>}
                </div>
              ) : (
                <motion.div
                  className={`font-display font-bold tracking-[-0.04em] ${
                    line.accent ? "text-white/30 italic" : "text-white"
                  } ${
                    i === 2
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 sm:gap-12">
          {/* Left: description — LCP element.
              Rendered as plain <p> on ALL devices so the browser sees it in the
              initial HTML parse with no JS delay. CSS handles the fade on desktop. */}
          <p className="text-[15px] sm:text-base font-sans text-white/40 max-w-[340px] leading-relaxed hero-desc">
        ...
        {/* Anchor scroll indicator — desktop only */}
        {!isMobile && (
          <motion.div
